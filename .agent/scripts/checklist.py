...
def print_summary(results: List[dict]):
    ...
    critical_failed = sum(1 for r in results if not r["passed"] and not r.get("skipped") and r.get("required", True))
    if critical_failed > 0:
        print_error(f"{critical_failed} critical check(s) FAILED - Please fix before proceeding")
        return False
    else:
        if failed_count > 0:
            print_warning(f"Some optional checks failed ({failed_count} failed), but all CRITICAL checks PASSED ✨")
        else:
