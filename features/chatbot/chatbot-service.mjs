/**
 * Advanced ERP Chatbot Service (ES Module Version)
...
try {
  ...
  if (!apiKey) {
    ...
    console.warn('Please set ANTHROPIC_API_KEY in your .env file');
  } else {
  ...
  try {
    ...
    if (!apiKeyConfigured || !anthropic) {
      return res.status(503).json({
        success: false,
        error: 'API Key chưa được cấu hình. Kiểm tra file .env - phần ANTHROPIC_API_KEY',
        hint: 'Truy cập https://console.anthropic.com để lấy API key'
      });
    }
...
app.get('/api/chat/status', (req, res) => {
  return res.json({
    success: true,
    status: apiKeyConfigured ? 'online' : 'offline',
...
app.listen(PORT, () => {
  console.log('\n╔════════════════════════════════════════╗');
  console.log('║     🤖 VIETBACHCORP Chatbot Service  ║');
  console.log('╚════════════════════════════════════════╝\n');
  console.log(`✅ Server running on port ${PORT}`);
  console.log(`📡 API: http://localhost:${PORT}/api/chat/message`);
  console.log(`🤖 Model: Claude 3.5 Sonnet`);
  console.log(`🔑 API Key configured: ${apiKeyConfigured ? '✅ Yes' : '❌ No (check .env file)'}`);
