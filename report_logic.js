...
function renderBaoCaoMuaHang() {
    if (window.erpApp.updateBreadcrumb) {
        window.erpApp.updateBreadcrumb('Báo cáo mua hàng', 'Mua hàng');
    }

    // Fake Data computations
    const purchaseOrders = window.erpApp.getPurchaseOrders ? window.erpApp.getPurchaseOrders() : [];
    ...
    // window.erpApp.formatValue will be used directly
    ...
    let html = `
            <div class="sales-order-module">
                <div class="so-toolbar" style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; gap:16px; flex-wrap:wrap;">
                    ...
                    <div style="display:flex; gap:12px;">
                        <button onclick="window.erpApp.sendReportToZalo()" style="padding:10px 24px; background:linear-gradient(135deg, #0068ff 0%, #0046ab 100%); color:#fff; border:none; border-radius:10px; font-weight:700; font-size:14px; display:flex; align-items:center; gap:8px; cursor:pointer; box-shadow:0 4px 12px rgba(0, 104, 255, 0.2);">
                            <span class="material-icons-outlined">send</span> Gửi Zalo
                        </button>
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(250px, 1fr)); gap:20px; margin-bottom:24px;">
                    <div style="background:#fff; border:1px solid #e2e8f0; border-radius:16px; padding:24px; box-shadow:0 4px 20px rgba(0,0,0,0.02); display:flex; flex-direction:column; gap:10px;">
                        ...
                        <div style="font-size:28px; font-weight:900; color:#10b981;">${window.erpApp.formatValue(totalSpent)}</div>
            ...
            topSuppliers.map((s, idx) => `
                                    <tr style="border-bottom:1px solid #f1f5f9;">
                                        ...
                                        <td style="padding:16px 24px; text-align:right; font-weight:800; color:#10b981;">${window.erpApp.formatValue(s.amount)}</td>
                                    </tr>
                                  `).join('')}
                            </tbody>
                        </table>
...
window.erpApp.simulatePrintPurchasingReport = function () {
    window.erpApp.showToast('Đang kết xuất Báo cáo Hệ thống...');
    setTimeout(() => {
        window.erpApp.showToast('✅ Đã tải xuống file Baocao_Muahang_VIETBACH.pdf !');
    }, 1200);
};

window.erpApp.sendReportToZalo = async function () {
    if (!window.ZaloService) {
        window.erpApp.showToast('❌ Dịch vụ Zalo chưa sẵn sàng!', 'error');
        return;
    }

    // 1. Tính toán dữ liệu báo cáo nhanh
    const purchaseOrders = window.erpApp.getPurchaseOrders ? window.erpApp.getPurchaseOrders() : [];
    const totalSpent = purchaseOrders.reduce((sum, po) => sum + (po.status !== 'cancelled' ? po.totalAmount : 0), 0);
    const totalPO = purchaseOrders.length;
    ...
    // 2. Soạn tin nhắn
    const msg = '📊 *BÁO CÁO MUA HÀNG - VIETBACH ERP*\n' +
                `📅 Ngày: ${new Date().toLocaleDateString('vi-VN')}\n` +
                '--------------------------\n' +
                `💰 Tổng chi phí: ${window.erpApp.formatValue(totalSpent)}\n` +
                `📦 Tổng số đơn: ${totalPO} phiếu\n` +
                `🏢 NCC lớn nhất: ${topSupplier ? topSupplier.name : 'N/A'}\n` +
                '--------------------------\n' +
                '📱 Xem chi tiết tại hệ thống VIETBACCORP.';

    // 3. Gửi tin
    window.erpApp.showToast('Đang gửi báo cáo qua Zalo...');
    try {
        const result = await window.ZaloService.sendMessage(msg);
        if (result && result.error === 0) {
            window.erpApp.showToast('✅ Đã gửi báo cáo qua Zalo!');
        } else {
            const errorMsg = result?.message || 'Lỗi kết nối Zalo';
            window.erpApp.showToast(`❌ Thất bại: ${errorMsg}`, 'error');
        }
    } catch (e) {
        window.erpApp.showToast('❌ Lỗi hệ thống khi gửi Zalo', 'error');
    }
};

window.erpApp.renderBaoCaoMuaHang = renderBaoCaoMuaHang;
