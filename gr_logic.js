(function () {
    ...
    let grSearchQuery = '';
    let grActiveTab = 'all';
    const COLLECTION_GR = 'erp_goodsReceipts';
    let goodsReceipts = (window.erpApp && window.erpApp._getData) ? window.erpApp._getData(COLLECTION_GR) : [];

    function renderKiemTraNhapHang() {
        if (window.erpApp.updateBreadcrumb) {
            window.erpApp.updateBreadcrumb('Kiểm tra nhập hàng', 'Mua hàng');
        }
            ...
            <div class="sales-order-module">
                ...
                <div class="crm-chevron-tabs">
                    ${['all', 'pending', 'partial', 'completed', 'rejected'].map(tab => {
                        const searchMatch = goodsReceipts.filter(r => {
                            const q = grSearchQuery.toLowerCase();
                            return r.id.toLowerCase().includes(q) || (r.poId || '').toLowerCase().includes(q);
                        });
                        const count = tab === 'all' ? searchMatch.length : searchMatch.filter(c => c.status === tab).length;
                        const isActive = grActiveTab === tab;
                        const label = tab === 'all' ? 'Tất cả' : window.erpApp.getGrStatusLabel(tab);
                        
                        let bgColor, textColor, badgeColor = '#1e293b';
                        if (tab === 'all') {
                            bgColor = isActive ? '#059669' : '#64748b';
                            textColor = '#fff';
                            badgeColor = isActive ? '#059669' : '#1e293b';
                        } else {
                            const colors = window.erpApp.getGrStatusColor(tab);
                            if (isActive) {
                                bgColor = colors.text; 
                                textColor = '#fff';
                        ...
                        return `
                            <button class="crm-chevron-tab" onclick="window.erpApp.grSetTab('${tab}')" 
                    ...
                    <div class="modal-body" style="background: var(--bg-body); padding:24px; max-height:70vh; overflow-y:auto;">
                        <div class="premium-card" style="margin-bottom:20px;">
                            ...
                            <div style="display:grid; grid-template-columns:2fr 1fr 1fr; gap:16px; margin-top:16px;">
                                <div class="form-group">
                                    <label>ĐƠN VỊ GIAO HÀNG / ĐỐI TÁC <span style="color:red;">*</span></label>
                                    <select name="supplier" required class="form-control" style="font-weight:700; color:#059669;">
    ...
    window.erpApp.saveGR = function(e) {
        ...
        window.erpApp._setData(COLLECTION_GR, goodsReceipts);
        
        // Sync to Firebase
        if (window.CrudSync && window.CrudSync.saveItem) {
            window.CrudSync.saveItem(COLLECTION_GR, newR, 'id');
        }
        ...
        if (window.erpApp.addNotification) {
            window.erpApp.addNotification(
                index > -1 ? `Đã cập nhật phiếu kiểm tra: ${newR.id}` : `Đã lập phiếu kiểm tra hàng: ${newR.id}`,
                'fact_check',
        ...
        window.erpApp.showToast('Đã lưu dữ liệu Phiếu Kiểm Tra Hàng!', 'success');
    ...
    window.erpApp.confirmDeleteGR = function(id) {
        window.erpApp.showConfirm(
            'Xác nhận hủy',
            'Bạn có chắc chắn muốn hủy Biên bản nhận hàng ' + id + '?',
            async function() {
                ...
                if (window.CrudSync && window.CrudSync.deleteItem) {
                    await window.CrudSync.deleteItem(COLLECTION_GR, id);
                }

                // Gửi thông báo hệ thống
                if (window.erpApp.addNotification) {
                    window.erpApp.addNotification(
                        `Đã xóa/hủy phiếu kiểm tra: ${id}`,
                        'delete',
                        'red',
                        'mua-hang'
                    );
                ...
                window.erpApp.showToast('Đã xóa dữ liệu thành công!', 'info');
                renderKiemTraNhapHang();
            }
        );
    };
    ...
    window.erpApp.renderKiemTraNhapHang = renderKiemTraNhapHang;
