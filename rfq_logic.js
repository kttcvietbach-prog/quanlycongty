(function () {
    ...
    const COLLECTION_RFQ = 'erp_rfqs';
    let rfqs = (window.erpApp && window.erpApp._getData) ? window.erpApp._getData(COLLECTION_RFQ) : [];

    function renderRFQ() {
        if (window.erpApp.updateBreadcrumb) {
            window.erpApp.updateBreadcrumb('Yêu cầu báo giá (RFQ)', 'Mua hàng');
        }
            ...
            <div class="sales-order-module">
                ...
                <div class="crm-chevron-tabs">
                    ${['all', 'draft', 'sent', 'received', 'closed'].map(tab => {
                        const searchMatch = rfqs.filter(r => {
                        ...
                        const isActive = rfqActiveTab === tab;
                        const label = tab === 'all' ? 'Tất cả' : window.erpApp.getRfqStatusLabel(tab);
                        
                        let bgColor, textColor, badgeColor = '#1e293b';
                        if (tab === 'all') {
                            bgColor = isActive ? '#8b5cf6' : '#64748b';
                            textColor = '#fff';
                            badgeColor = isActive ? '#8b5cf6' : '#1e293b';
                        } else {
                            const colors = window.erpApp.getRfqStatusColor(tab);
                            if (isActive) {
                                bgColor = colors.text; 
                                textColor = '#fff';
                        ...
                        return `
                            <button class="crm-chevron-tab" onclick="window.erpApp.rfqSetTab('${tab}')" 
    ...
    window.erpApp.simulateSendRfqEmail = function(id) {
        window.erpApp.showToast('🔄 Đang kết nối máy chủ Mail...');
        setTimeout(() => {
            window.erpApp.showToast('✅ Đã gửi Email Yêu cầu Báo Giá (RFQ) tới Nhà cung cấp cho luồng: ' + id);
            const index = rfqs.findIndex(x => x.id === id);
            if (index > -1 && rfqs[index].status === 'draft') {
                rfqs[index].status = 'sent';
                window.erpApp._setData(COLLECTION_RFQ, rfqs);
                renderRFQ();
            }
        }, 1500);
    };
                    ...
                    <div class="modal-body" style="background: var(--bg-body); padding:24px; max-height:70vh; overflow-y:auto;">
                        <div class="premium-card" style="margin-bottom:20px;">
                            ...
                            <div class="form-group" style="margin-top:16px;">
                                <label>GỬI TỚI NHÀ CUNG CẤP <span style="color:red;">*</span></label>
                                <select name="supplier" required class="form-control" style="font-weight:700; color:var(--primary);">
                                    <option value="">-- Chọn nhà cung cấp --</option>
                                    ${(window.erpApp.getSuppliers ? window.erpApp.getSuppliers() : []).map(s => `
                                        <option value="${s.name}" ${r.supplier === s.name ? 'selected' : ''}>${s.name} (${s.id})</option>
                                    `).join('')}
                                </select>
                            </div>
    ...
    window.erpApp.saveRFQ = async function(e) {
        e.preventDefault();
        ...
        if (window.CrudSync && window.CrudSync.saveItem) {
            await window.CrudSync.saveItem(COLLECTION_RFQ, newR, 'id');
        }
        ...
        if (window.erpApp.addNotification) {
            window.erpApp.addNotification(
                index > -1 ? `Đã cập nhật RFQ: ${newR.id}` : `Đã tạo yêu cầu báo giá mới: ${newR.id}`,
                'request_quote',
                index > -1 ? 'indigo' : 'blue',
                'mua-hang'
            );
        ...
        window.erpApp.showToast('Đã lưu YCBG ' + newR.id, 'success');
        renderRFQ();
    };
    ...
    window.erpApp.confirmDeleteRFQ = function(id) {
        window.erpApp.showConfirm(
            'Xác nhận xóa',
            'Bạn có chắc chắn muốn xóa Yêu cầu báo giá ' + id + ' khỏi hệ thống?',
            async function() {
                rfqs = rfqs.filter(x => x.id !== id);
                window.erpApp._setData(COLLECTION_RFQ, rfqs);
                
                if (window.CrudSync && window.CrudSync.deleteItem) {
                    await window.CrudSync.deleteItem(COLLECTION_RFQ, id);
                }
                ...
                window.erpApp.showToast('Đã xóa RFQ!');
                renderRFQ();
            }
        );
    };
    ...
    window.erpApp.renderRFQ = renderRFQ;
